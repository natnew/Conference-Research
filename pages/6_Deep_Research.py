...
class ReportUtils:
    ...
    def save_report(report: str, topic: str) -> str:
        ...
        filename = f"reports/{timestamp}_{topic.replace(' ', '_')[:30]}.md"
...
class ReportGenerator:
    ...
    def generate_report(
        ...
        try:
            ...
            # Save report
            report_file = ReportUtils.save_report(final_report, topic)
            
            # Complete progress
            progress_bar.progress(100)
            status_text.success(f"Research complete! Report saved to {report_file}")
...
def main():
    ...
    5. Watch progress in real-time and review the final report with citations.
        ...
        # Add ability to load previous report
        ...
        if os.path.exists("reports") and os.listdir("reports"):
            ...
            selected_report = st.selectbox(
                ...
                ["Select a report..."] + report_files
            )
            if selected_report != "Select a report...":
                if st.button("Load Report"):
                    report_content = ReportUtils.load_report(f"reports/{selected_report}")
                    st.session_state.loaded_report = report_content
    ...
    if "loaded_report" in st.session_state:
        report_placeholder.markdown(st.session_state.loaded_report, unsafe_allow_html=True)
